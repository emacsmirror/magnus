...
(ert-deftest magnus-coord-shutdown-releases-watchers-hooks-and-saves ()
  (let* ((poll (run-at-time 3600 nil #'ignore))
         ...
         (retry (run-at-time 3600 nil #'ignore))
         (event-retry (run-at-time 3600 nil #'ignore))
         ...
         (magnus-coord--processed-review-ready '(("/tmp/project" . ("x"))))
         (magnus-coord--review-ready-retries (make-hash-table :test #'equal))
         (magnus-coord--event-review-retries (make-hash-table :test #'equal))
         (window-buffer-change-functions nil)
         (pre-command-hook nil)
         (saves 0))
    (puthash '("/tmp/project" . "marker") (list :timer retry)
             magnus-coord--review-ready-retries)
    (puthash '("/tmp/project" . "event") (list :timer event-retry)
